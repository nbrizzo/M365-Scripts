<#
.SYNOPSIS
M365 Distribution Group Alias Addtion tool
Version 1.0
...
.DESCRIPTION
This script is used to add additional aliases to distribution groups post M365 migration cutover.
...
If (Get-InstalledModule 'ExchangeOnlineManagement'){
    ...
    foreach ($distro in $csv) 
    {
            ...
            $formattedaliases = $distro.aliases -split ','
            Write-Host "Adding aliases for distribution group $($distro.identity)"
            #Adds the formatted aliases to the current mailbox in the loop.
            Set-DistributionGroup -Identity $distro.identity -EmailAddresses @{add=$formattedaliases} -ErrorAction Stop
            $messagesmtp = "Adding aliases for distribution group $($distro.identity)"
            Write-LogMessage($messagesmtp)
            
        }
            catch{
            #Log failure operation to log file & continue to UPN update section.
            $FailureMessage = "There was an issue while updating the aliases for  $($distro.identity), please manually modify the SMTP settings of this user"
